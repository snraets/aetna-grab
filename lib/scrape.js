...
exports.scrape = async (page, location) => {
    ...
    await page.evaluate( () => {
        $.filter_helper.setSearchRadius('P', 5); //50        
    ...
    console.log('ZipCode: ', location.zipCode,  ' Number of doctors: ', doctorNumber, ' Iterations: ', doctorLoop - 1);

    if(doctorNumber === 0){        
        jsonfile.writeFileSync(`./results/results.${location.zipCode}.json`, []);
        await wait(1500);
        return;
    }
